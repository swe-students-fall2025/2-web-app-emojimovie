...
def insert_many_messages(messages_data: List[Dict[str, Any]]) -> List[str]:
    try:
        result = db.messages.insert_many(messages_data)
        return [str(_id) for _id in result.inserted_ids]
    except PyMongoError as e:
...
def find_one_message(filter: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    try:
        message = db.messages.find_one(filter)
        return message
    except PyMongoError as e:
        print(f"Error finding message: {e}")
        return None

def find_many_messages(filter: Dict[str, Any]) -> List[Dict[str, Any]]:
    try:
        messages = list(db.messages.find(filter))
        return messages
    except PyMongoError as e:
...
def update_many_messages(filter: Dict[str, Any], update_data: Dict[str, Any]) -> int:
    try:
        result = db.messages.update_many(filter, {"$set": update_data})
        return result.modified_count
    except PyMongoError as e:
...
def delete_many_messages(filter: Dict[str, Any]) -> int:
    try:
        result = db.messages.delete_many(filter)  
        return result.deleted_count
    except PyMongoError as e:
