...
def create_listing():
    ...
    return (
        jsonify({"ok": True, "message": "Listing created successfully!", "data": doc}),
        201,
    )
...
def update_listing(listing_id):
    ...
    set_fields = {}
    if "itemName" in data:
        set_fields["itemName"] = (data.get("itemName") or "").strip()
    if "description" in data:
        set_fields["description"] = (data.get("description") or "").strip()
    if "price" in data:
        set_fields["price"] = (data.get("price") or "").strip()
    ...
    if new_photo_obj and prev.get("photo", {}).get("url"):
        ...
        if old_url.startswith("/static/uploads/"):
            ...
            old_path = os.path.join(
                current_app.root_path, "static", "uploads", old_name
            )
            try:
    ...
    try:
        updated = listings_dal.update_one_listing({"_id": listing_id}, set_fields)
