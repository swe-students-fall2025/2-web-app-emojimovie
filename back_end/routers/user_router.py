...
def login():
    data = request.json
    user = users_dal.find_one_user({"email": data.get('email')})
...
def edit_profile(user_id):
    try:
        ...
        allowed_fields = ["name", "email", "preferredLocation", "socialMediaType", "socialMediaUsername"]
        update_data = {k: v for k, v in form_data.items() if k in allowed_fields and v}
        
        if not update_data:
            return jsonify({"error": "No valid fields to update"}), 400
        
        success = users_dal.update_one_user(
